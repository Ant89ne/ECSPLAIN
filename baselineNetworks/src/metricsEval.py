...
def getMetrics(camPred, truth, camTruth, threshold = 0.5):
    """
    ...
    with torch.no_grad():
        ...
        #MAE
        MeanAbs = nn.L1Loss()
        mae = MeanAbs(camPred, camTruth[0])
...
def compute_seg_mets(pred,truth,labs, thresh = 0.5, device = "cpu"):
    truth = truth[0]
    
    with torch.no_grad():
        nbones = torch.sum(labs)
        if not nbones:
            iou = torch.zeros(1)
            auroc = torch.zeros(1)
        else:
